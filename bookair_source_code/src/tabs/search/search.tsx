...
const Search: React.FC<Props> = ({
  ...
  const bestStr = "Round-Trip";
  const cheapestStr = "One-Way";
  ...
  function navigateToSearchResultScreen() {
    ...
    if (locationnFrom && locationnTo) {
      ...
      Alert.alert(
        "Error",
        (locationnFrom ? "Destination" : "Origin") + " info is missing.",
        [
          {
            text: "Select",
            onPress: () => {
    ...
    return (
      ...
      >
        ...
        <Text style={styles.flight} numberOfLines={1}>
          {airport.length == 0 ? "Any" : airport}
        </Text>
        <Text style={styles.date} numberOfLines={1}>
          {location.length == 0 ? "City, Country" : location}
        </Text>
      </TouchableOpacity>
    );
  }
  ...
  function getFlightView(
    ...
    return (
      <View style={styles.flightView}>
        ...
        {getFlight(airport2, location2, false)}
      </View>
    );
  }
